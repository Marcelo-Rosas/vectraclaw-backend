...
python start_server.py        # API em :3100 (sem --reload — restart manual quando código muda)
python start_all_daemons.py   # sobe os 8 daemons (cada um faz lock no AGENT_ID)
```

Health: `curl http://localhost:3100/api/health` retorna `{"status":"online"}`.
