...
from .m3_tools import TOOLS_REGISTRY
...
def execute_tool(name: str, payload: str = '') -> ToolExecution:
    # Se a tool requisitada for uma ferramenta nativa M3 (Logística)
    if name in TOOLS_REGISTRY:
        try:
            native_result = TOOLS_REGISTRY[name](payload)
            action = f"Native M3 Execution of '{name}' returned:\n{native_result}"
            return ToolExecution(name=name, source_hint="src/m3_tools.py", payload=payload, handled=True, message=action)
        except Exception as e:
            return ToolExecution(name=name, source_hint="src/m3_tools.py", payload=payload, handled=False, message=f"Native Tool Crash: {e}")

    # Fallback pro comportamento de Shim do Claw
