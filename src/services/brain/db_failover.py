"""
VEC-188 – Self-healing para failover de banco de dados.

Quando uma operação Supabase/Postgres falha, este módulo:
  1. Classifica o erro (FK, constraint, schema, auth, rede, etc.)
  2. Monta um `FailoverResult` com contexto suficiente para o agente corrigir e retentar
  3. Exporta o decorator `@with_db_failover` para wrapping transparente de corrotinas

O payload de self-healing é intencionalmente verbose — o agente precisa de tudo para
raciocinar sobre a correção sem precisar de uma segunda viagem ao servidor.
...
logger = logging.getLogger("brain.db_failover")
...
def with_db_failover(
    ...
    """
    Decorator para corrotinas async que fazem operações de banco.

    Captura exceções de banco, constrói um FailoverResult e lança
    uma HTTPException estruturada. O `detail` da exceção contém o
    contexto completo para o agente (ai_instruction, suggested_fix, etc.).

    Args:
        operation:    label da operação para o FailoverResult (ex: "insert:tasks")
        table:        tabela alvo
        retry_hint:   endpoint sugerido para retry
        reraise_http: se True (padrão), lança HTTPException; se False, retorna FailoverResult

    Uso:
        @with_db_failover(operation="insert:tasks", table="tasks", retry_hint="POST /api/companies/{id}/tasks")
