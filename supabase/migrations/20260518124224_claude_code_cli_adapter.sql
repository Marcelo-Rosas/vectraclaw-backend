...
BEGIN
  ...
  SELECT count(*) INTO v_fields FROM vectraclip.adapter_field_definitions afd
    ...
    WHERE ac.slug='claude_code_cli';
  RAISE NOTICE 'W8: claude_code_cli adapters=% | fields totais=% (esp 4 por adapter)',
    v_adapters, v_fields;
  IF v_adapters < 1 THEN
    RAISE EXCEPTION 'W8: nenhum adapter claude_code_cli criado — esperado >=1';
  END IF;
END $$;
